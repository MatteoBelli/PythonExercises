...
class Test(testlib.TestCase):
    ...
    def do_test(self, ls, testo, expected_lista, expected_parola, expected_ls):
        ...
        self.check(len(res), 2, None,
                   'non viene restituita una tupla di 2 elementi')
        lista, parola = res
        # print('\n\n\tls',len(ls),'lista', len(expected_lista), 'parola', len(expected_parola), 'ls', len(expected_ls))
        self.check(lista, expected_lista, None,
                   'non viene restituita la lista corretta')
        self.check(parola, expected_parola, None,
                   'non viene restituita la parola corretta')
